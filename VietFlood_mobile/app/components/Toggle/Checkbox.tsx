...
function CheckboxInput(props: CheckboxInputProps) {
  ...
  const iconTintColor = [
    disabled && colors.palette.neutral500,
    status === "error" && colors.error,
    colors.palette.neutral100,
  ].filter(Boolean)[0]
  ...
  return (
    ...
    >
      ...
      >
        {(() => {
          const IconComponent = icon ? iconRegistry[icon] : iconRegistry.check
          return <IconComponent color={iconTintColor as string} size={20} />
        })()}
      </Animated.View>
    </View>
  )
