...
function SwitchInput(props: SwitchInputProps) {
  ...
  const onBackgroundColor = [
    ...
    status === "error" && colors.errorBackground,
    colors.success, // Apple Green typically for success
  ].filter(Boolean)[0]
  ...
  return (
    <View style={$switchAccessibilityStyle}>
      ...
      {accessibilityMode === "icon" &&
        ...
        (() => {
          const IconComponent = role === "off" ? iconRegistry.hidden : iconRegistry.view
          return (
            <View style={[$switchAccessibilityIcon, { tintColor: color }]}>
              <IconComponent color={color as string} size={20} />
            </View>
          )
        })()}
    </View>
...
const $inputOuter: StyleProp<ViewStyle> = [
  $inputOuterBase,
  // Apple standard switch dimensions
  { height: 31, width: 51, borderRadius: 15.5, borderWidth: 0 },
]
